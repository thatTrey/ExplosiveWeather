...
class MineSweeperBoard():
  ...
  def Player_Reveal_Guess(self, x, y):
    ...
    #checks for bomb
     if(self.board_Array[x][y] == -1): #if there is a bomb
      #trips loss conditions
      Win_Loss_Flag = -1
      return -1
     else: 
        if(self.board_Array[x][y] == 0): # if there are no bombs adjacent, clear area on board
           MineSweeperBoard.__Board_Bombless_Clear(self, x, y)
           print("delete later")
